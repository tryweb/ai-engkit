...
Surfaces per-project codegraph index health and per-project LeanCTX knowledge summaries through the Admin projects overview API and UI, and site-level leanCTX statistics through the Admin Dashboard, so an admin can see at a glance whether a workspace project is indexed for code intelligence and how leanCTX context is used across the workspace.
